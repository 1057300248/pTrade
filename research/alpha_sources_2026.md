...
## 六、17 行业 GEM 不是 Dual Momentum；下一个实盘候选改为 `ptrade_adm_etf.py`

对第五节路线的修正：把 17 只行业 ETF 装进"GEM"外壳，得到的不是 Dual Momentum。

- **为什么 17 行业 GEM 不成立**：Antonacci 双动量的前提是**少量、低相关、经济逻辑上互斥的资产类别**之间做相对动量，再用绝对动量门控制回撤。17 只行业 ETF 相关性高（2026 上半年 K 型行情下几乎同涨同跌），绝对动量门同开同关、不提供独立信息，相对动量退化成行业截面排序——正是第五节已判死刑的形态换了个名字。行业颗粒度的轮动不再作为实盘候选。
- **下一个实盘候选：`ptrade_adm_etf.py`**（Antonacci 式双动量，资产类别颗粒度）。风险腿 4 条：沪深 300（510300）、创业板（159915）、纳指 QDII（513100）、黄金（518880）；安全腿：债券 ETF。信号 = **12-1 相对动量**（约 250 日收益、跳过最近 21 日）在风险腿间选最强 + **21 日绝对动量门**（最强腿近 21 日收益跑输债券腿或 ≤ 0 时全仓退到债券）。腿数少、逻辑互斥，绝对动量门才有独立的防崩功能。
- **513100 的定位**：QDII 纳指在这里是**资产类别 leg**（与 A 股低相关的第二权益极，2026-07 A 股动量踩踏期间提供跨市场分散），**不是** RMDC 混合池里的"分散器"——那种把它当截面候选打分的用法已随第五节判决废弃。注意 QDII 溢价与限购风险：溢价率异常时该腿降权或跳过。
- **保留的 overlay 不变**：拥挤度否决（信号 2，四指标 1250 日分位）+ 崩盘保护 overlay 继续覆盖所有权益腿，与 ADM 的绝对动量门并联而非替代——前者防拥挤出清，后者防趋势崩塌。
- **term_spread 维持 research-only**：动量期限差（信号 3）仍未通过纯股票池独立 walk-forward，不进 `ptrade_adm_etf.py`，也不进任何实盘文件。
