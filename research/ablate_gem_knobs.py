...
VARIANTS = build_grid()
UNIVERSE = tuple(dict.fromkeys(list(GROWTH) + list(DIVERSIFIER)))
...
def reopening_verdict(frozen):
    sharpe_pass = not frozen["overfit"]
    july_return = frozen["stress"]["2026-07"]["return"]
    stress_pass = july_return > -0.10
    if sharpe_pass and stress_pass:
        return (
            "REOPEN as UNDER TEST: the frozen IS winner passes both the OOS "
            "Sharpe-decay rule and the 2026-07 > -10% stress rule. This is "
            "not approval to make GEM the automatic live primary."
        )
    failed = []
    if not sharpe_pass:
        failed.append("OOS Sharpe decay")
    if not stress_pass:
        failed.append("2026-07 stress return")
    return "REJECTED: the frozen IS winner fails %s." % " and ".join(failed)
...
def render_report(results, frozen):
    ...
    verdict = reopening_verdict(frozen)
...
def run():
    ...
    print(reopening_verdict(frozen))
